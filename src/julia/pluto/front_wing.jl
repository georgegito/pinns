...
# ╔═╡ ef2bd1db-165b-409e-bb97-017f2e6f0c8c
...
# ╔═╡ e63dabad-caf8-447e-9df4-2f67c458be25
begin
	u = [u_df.x, u_df.y, u_df.z]
end;

# ╔═╡ 19f6d925-c411-40c1-b1db-4f88ce47fa8b
num_of_samples = length(u[1])

# ╔═╡ c24f9fb3-a31c-4c45-8b42-80e6b98b0215
indices = sample(1:num_of_samples, 10000, replace=false);

# ╔═╡ 2763caa3-2ff4-43a0-81d4-73bfe010322f
...
# ╔═╡ 01f792c8-6ffa-4051-8a5e-45d0bd4c45ca
u_sampled = [u[1][indices], u[2][indices], u[3][indices]];

# ╔═╡ 262b3cb6-5191-4d69-a663-749d065d1244
min = minimum(minimum.(u_sampled))

# ╔═╡ b5e77f81-db0d-42ef-aaa8-100259d24759
max = maximum(maximum.(u_sampled))

# ╔═╡ 6ee57642-4d13-43be-b540-6ed0b4d277fa
range = (min, max)

# ╔═╡ b794b7c6-5032-406d-8802-a3f2c03f95f9
begin
	plotly()
	scatter3d(u_sampled[1], u_sampled[2], u_sampled[3], markersize=0.5, color=:purple, xlims=range, ylims=range, zlims=range)
end

# ╔═╡ dc70e7e4-9201-4c23-9291-f4b85a3f4bba
u_mat = [u_sampled[1] u_sampled[2] u_sampled[3]]
...
begin
	# Plot the points
	plotly()  # Use Plotly backend for interactive 3D plots
	scatter3d(u_norm[:, 1], u_norm[:, 2], u_norm[:, 3], markersize=0.5, color=:purple, xlims=[0, 1], ylims=[0, 1], zlims=[0, 1])
end
...
# ╔═╡ 6609e58a-58d3-4b7b-b489-a1de51d92072
u_scaled_mat = [u_scaled_df.x u_scaled_df.y u_scaled_df.z]
...
# ╔═╡ 0fca1593-df6b-46d9-ab1e-bec99de1841e
tr_max = maximum(maximum.([u_tr_df.x, u_tr_df.y, u_tr_df.z]))
...
begin
	plotly()
	scatter3d(u_unif_scaled_df.x, u_unif_scaled_df.y, u_unif_scaled_df.z, markersize=0.5, color=:purple, xlims=[0, 1], ylims=[0, 1], zlims=[0, 1])
end
...
# 	# Plot the points
# 	plotly()  # Use Plotly backend for interactive 3D plots
# 	scatter3d(u[:, :], markersize=0.5, color=:purple, xlims=x_limits, ylims=y_limits, zlims=z_limits)
# end
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "a39f3a84b765a1b61057bd4eef6df60c4354cbfd"
...
git-tree-sha1 = "367baf3c845920976f66af138dc243a162d32376"
...
git-tree-sha1 = "d3aef841063140b021b5e676c15ace852f317e0f"
